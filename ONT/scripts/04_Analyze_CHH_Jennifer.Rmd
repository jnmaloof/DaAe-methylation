...
```
```{r}
L121A <- readGeneric( "../output/L121A01.gz",
                    meta.cols=list(L121A_score=11,
                                   L121A_coverage=10))

L121A <- L121A[seqnames(L121A) %in% chrkeep] 
seqlevels(L121A) <- seqlevelsInUse(L121A)
L121A
```

```{r}
L121C <- readGeneric( "../output/L121Cchr.gz",
                    meta.cols=list(L121C_score=11,
                                   L121C_coverage=10))

L121C <- L121C[seqnames(L121) %in% chrkeep] 
seqlevels(L121C) <- seqlevelsInUse(L121)
L121C
```

```{r}
T26 <- readGeneric("../output/T26_ONT_to_DaAe_2024-15-05chh.bed.gz",
...
```{r}
DaAe <- readGeneric("../output/DaAe_ONT_to_DaAe_2024chh.bed.gz"   ,
