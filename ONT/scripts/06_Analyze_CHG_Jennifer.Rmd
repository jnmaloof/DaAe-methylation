...
```{r}
L121 <- readGeneric("../output/L121_ONT_to_DaAe_filtered1.bed.gz",
...
```{r}
T26 <- readGeneric("../output/T26_ONT_to_DaAe_filtered.bed.gz",
...
```{r}
DaAe <- readGeneric("../output/DaAe_ONT_to_DaAe_filtered.bed.gz",
...
```{r}
All.df %>%
  pivot_longer(c(L121, T26, DaAe), values_to = "CHG", names_to = "ID") %>%
  ggplot(aes(x=position, y=CHG, color = ID)) +
  geom_smooth() +
